...
function exec(message, args){
    ...
    const member = this.client.util.resolveMember(args.content, message.guild, false, true);
    if (member) return message.edit(`\`${member.id}\``);

    const channel = this.client.util.resolveChannel(args.content, message.guild, false, true);
    if (channel) return message.edit(`\`${channel.id}\``);

    const role = this.client.util.resolveRole(args.content, message.guild, false, true);
    if (role) return message.edit(`\`${role.id}\``);
    
    const emoji = this.client.util.resolveEmoji(args.content, message.guild, false, true);
